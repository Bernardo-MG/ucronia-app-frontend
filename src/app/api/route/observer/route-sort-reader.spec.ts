...
describe('RouteSortReader', () => {
  ...
  it('should provide an empty sort when there is no parameters', () => {
  ...
  it('should be able to parse a single sort', () => {
  ...
  it('should be able to parse multiple sorts', () => {
    ...
    expect(request[0].property).toEqual('property');
    expect(request[0].order).toEqual('asc');
    expect(request[1].property).toEqual('property1');
    expect(request[2].order).toEqual('desc');
  });
